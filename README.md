...
本地自动化或第三方工具可以通过命令行导出聚合后的使用统计：

```bash
TrackMyMac --summary-json today
TrackMyMac --summary-json 7d
TrackMyMac --summary-json 30d
TrackMyMac --summary-json 365d
```

输出只包含聚合统计，例如按键次数、点击次数、移动距离、活跃时长、亮屏时长、Top Apps 和按键类别计数。该命令不会导出 `keystrokes.cipher`，也不会解密或输出具体输入内容。
